...
const int D_min = 40; // distancia minima
const int delay_giro = 400;
const int delay_atras = 500;
const int delay_servo = 400;
const int delay_wat = 100;
const int delay_180 = 400;

unsigned long ahora = 0;     // tiempo actual
unsigned long antes = 0;     // tiempo pasado
...
void deteccion_colision(){
  ...
  }else{  // si la distancia no esta entre 0 y 20... ir adelante
    ir_adelante(vel_izq, vel_der);
    /* while(ac_izquierdo < 1){
      ir_adelante(vel_izq, 0);
    }
    while(ac_derecho < 1){
      ir_adelante(0, vel_der);
    }
    ac_derecho = 0;
    ac_izquierdo = 0; */
  }
...
void setup(){
  ...
  attachInterrupt(digitalPinToInterrupt(2), isr_motor_derecho, CHANGE);
  attachInterrupt(digitalPinToInterrupt(3), isr_motor_izquierdo, CHANGE);
  ac_derecho = 0;
  ac_izquierdo = 0;
}
